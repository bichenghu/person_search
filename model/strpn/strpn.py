...
class STRPN(nn.Module):
    ...
    def __init__(self, net_conv_channels, num_pid, is_train=False):
        """
        ...
        #self.proposal_target_layer = ProposalTargetLayer(self.config, self.num_pid)

        self.roi_pooling = ROIPool((self.pooling_size, self.pooling_size), 1.0/16.0)
        self.roi_align = ROIAlign((self.pooling_size, self.pooling_size), 1.0/16.0, 2)
    ...
    def forward(self, head_features, gt_boxes, im_info, mode='gallery'):
        if self.is_train:
            if mode == 'gallery':
                ...
                pooled_feat = self.pooling(head_features, rois, max_pool=False)#self.roi_pooling(head_features, rois)
                ...
                pooled_feat = self.roi_pooling(head_features, gt_boxes)
                return pooled_feat
            ...
            if mode == 'gallery':
                ...
                pooled_feat = self.roi_pooling(head_features, rois)
                transformed_feat = spatial_transform(
                    pooled_feat, roi_trans_param)
    ...
    def proposal_target_layer(self, rpn_rois, rpn_scores, trans_param,
        ...
        all_scores = torch.cat((all_scores.unsqueeze(1), zeros), 0)
